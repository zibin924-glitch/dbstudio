...
class DocGenerator:
    ...
    def generate_pdf(
        ...
        """Generate a PDF document from table metadata.

        Uses markdown-to-HTML conversion and then writes the HTML as a
        self-contained PDF-friendly file. For production use, integrate
        a proper HTML-to-PDF library (e.g., weasyprint).
        ...
        # Write as HTML (which can be printed to PDF from a browser)
        with open(output_path, "w", encoding="utf-8") as f:
            f.write(html_content)

        logger.info("PDF (HTML) document saved to %s", output_path)
